...
public class SectionProgressBar extends View{
    private static final long DEFAULT_DRAW_CUSOR_INTERNAL = 500; //游标绘制间隔
    private static final float DEFAULT_CURSOR_WIDTH = 4f;  // 游标宽度
    private static final float DEFAULT_BREAK_POINT_WIDTH = 2f;  // 断点宽度
    private static final long DEFAULT_FIRST_POINT_TIME = 2 * 1000; // 最少拍2s
    private static final long DEFAULT_TOTAL_TIME = 15 * 1000;  // 总时长15s
    ...
    private void init(Context paramContext) {
        ...
        setBackgroundColor(Color.parseColor("#22008080"));

        mProgressBarPaint.setStyle(Paint.Style.FILL);
        mProgressBarPaint.setColor(Color.parseColor("#FF00FF"));
        ...
        mFirstPointPaint.setColor(Color.parseColor("#FF1493"));
    ...
    public void setTotalTime(Context context, long millisecond) {
        ...
        mPixelUnit = dm.widthPixels / mTotalTime;  // 每一毫秒对应的像素
        ...
        synchronized (this) {
            if (!mBreakPointInfoList.isEmpty()) {
                ...
                for (BreakPointInfo info : mBreakPointInfoList) {
                    ...
                    canvas.drawRect(left, 0, startPoint, getMeasuredHeight(), mProgressBarPaint); // ProgressBar总长度
                    canvas.drawRect(startPoint, 0, startPoint + DEFAULT_BREAK_POINT_WIDTH, getMeasuredHeight(), mBreakPointPaint);  // 已经录制的长度
                    startPoint += DEFAULT_BREAK_POINT_WIDTH; // 绘制起点
